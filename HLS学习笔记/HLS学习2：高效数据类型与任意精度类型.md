...
modified: '2025-07-28T08:54:34.719Z'
---
...
ap_int<6> a_6bit_var_c(-22);
ap_int<6> a_6bit_var_c_r2("0b101010",2);
ap_int<6> a_6bit_var_c_r8("0o52",8);
ap_int<6> a_6bit_var_c_r10("-22",10);
ap_int<6> a_6bit_var_c_r16("0x2A",16);
...
ap_[u]fixed<W,I,Q,O,N>类型用于定义定点变量，
W 表示整个数据字长，I 表示整数部分数据字长，W-I 则为小数部分字长。
Q 表示低位部分的量化（Quantization）模式，默认为 AP_TRN_ZER（Truncation to zero）；
O 表示溢出（Overflow）模式，默认为 AP_WRAP（Wrap around）。
...
...
```
上面的例子中的AP_RND是类型标识，UG902的表8中提供了 ap_fixed 类型的标识的汇总。
